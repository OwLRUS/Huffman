...
Node* AllocNode(char letter, unsigned int frequency, Node* left, Node* right) 
{
    Node* node = new Node();
    node->letter = letter;
...
void HuffmanDecoder(const char* input_text = "chipher.txt", const char* output_text = "plain text.txt") {
    ...
    if (!input)
    {
        puts("HuffmanDecoder ERROR: Output text does not open\n");
        exit(1);
    }
...
void AreEqual(const char* text_name1 = "text.txt", const char* text_name2 = "plain text.txt")
{
